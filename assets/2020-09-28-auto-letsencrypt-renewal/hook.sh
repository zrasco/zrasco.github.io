...
deploy_challenge() {
  ...
  echo -n " - Setting TXT record with GoDaddy _acme-challenge${SUBDOMAIN}=${TOKEN_VALUE}"
...
clean_challenge() {
  ...
  local DOMAIN="${1}" TOKEN_FILENAME="${2}" TOKEN_VALUE="${3}"
  echo -n " - Removing TXT record from GoDaddy _acme-challenge${SUBDOMAIN}=--removed--"
  curl -X PUT https://api.godaddy.com/v1/domains/${DOMAIN}/records/TXT/_acme-challenge${SUBDOMAIN} \
    -H "Authorization: sso-key ${GODADDY_KEY}:${GODADDY_SECRET}" \
    -H "Content-Type: application/json" \
...
deploy_cert() {
  echo "deploy_cert() executing."
  echo "deploy_cert() done."
}
